...
import LatePassHandler from "./LatePassHandler";
...
export default async function FundingCommonsResidencyApplicationPage() {
  ...
  const userMentorRole = session?.user ? await db.userRole.findFirst({
    where: {
  ...
  // Check if user is admin (bypass latePass requirement)
  const isAdmin = session?.user?.role === "admin" || session?.user?.role === "staff";

  // Bypass latePass if user is mentor or admin
  const shouldBypassLatePass = !!userMentorRole || isAdmin;
  ...
  // If user is mentor or admin, show application directly without latePass check
  if (shouldBypassLatePass) {
    return (
      <div>
        {/* Info Banner */}
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-b border-blue-200 px-4 py-4">
  ...
  return (
    <LatePassHandler fallback={closedMessage}>
      <div>
        {/* Info Banner */}
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 border-b border-blue-200 px-4 py-4">
          <div className="container mx-auto max-w-6xl">
