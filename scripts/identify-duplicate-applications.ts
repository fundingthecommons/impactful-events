...
async function identifyDuplicates(targetEmail?: string) {
  ...
  try {
    ...
    console.log(`📊 Total applications${emailFilter}: ${allApplications.length}`);
    ...
    for (const [emailEvent, apps] of duplicates) {
      ...
      sortedApps.forEach((app, index) => {
        ...
        console.log(`      📊 ${app.responseCount} responses | ${linkStatus} | ${app.source} | ${app.status}`);
        console.log(`      📅 Created: ${app.createdAt.toLocaleDateString()} ${app.createdAt.toLocaleTimeString()}`);
      });
      ...
      console.log(`   🎯 Would merge into: ${keeperApp.id} (${keeperLinkStatus})\n`);
    }

    console.log(`\n🎯 Summary:`);
    console.log(`   📧 ${duplicates.length} email addresses have duplicates`);
    console.log(`   📋 ${duplicates.reduce((sum, [, apps]) => sum + apps.length, 0)} total duplicate applications`);
    console.log(`   🔗 ${duplicates.filter(([, apps]) => apps.some(app => app.hasUserId)).length} have at least one linked application`);
    console.log(`   📦 ${duplicates.filter(([, apps]) => apps.every(app => !app.hasUserId)).length} have only unlinked applications`);
