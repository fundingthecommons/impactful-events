...
async function analyzeApplications(): Promise<AnalysisResult[]> {
  ...
  for (const application of draftApplications) {
    ...
    try {
      ...
      console.log(`  👤 User: ${result.userName ?? 'Unknown'} (${result.userEmail})`);
      console.log(`  📅 Submitted: ${result.submittedAt.toLocaleString()}`);
      console.log(`  ✅ Complete: ${result.isComplete} (${result.completionPercentage}%)`);
      if (!result.isComplete) {
        console.log(`  ❌ Missing: ${result.missingFields.join(', ')}`);
      }
      console.log(`  🚀 Should be SUBMITTED: ${result.shouldBeSubmitted ? 'YES' : 'NO'}`);
...
function generateSummaryReport(results: AnalysisResult[]) {
  ...
  console.log(`📋 Total DRAFT applications with submittedAt: ${totalApplications}`);
  console.log(`✅ Complete applications (should be SUBMITTED): ${completeApplications}`);
  console.log(`❌ Incomplete applications (remain DRAFT): ${incompleteApplications}`);
  ...
  if (shouldBeSubmitted > 0) {
    ...
    console.log("📋 Next: Run Phase 2 script to update these applications to SUBMITTED status");
  } else {
