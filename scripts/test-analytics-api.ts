...
class AnalyticsAPITester {
  ...
  async testTextCorpusEndpoint(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Success: ${result.participantCount} applications, ${Object.keys(response.questionTypes).length} question types`);
      console.log(`📊 Response size: ${(result.dataSize / 1024).toFixed(1)}KB in ${result.responseTime}ms`);
      ...
      if (error.code === 'TOO_MANY_REQUESTS') {
        ...
        console.log('🔒 Access denied - check user role (requires researcher/staff/admin)');
      } else {
  ...
  async testDemographicsEndpoint(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Success: ${result.participantCount} participants analyzed`);
      console.log(`👨‍👩‍👧‍👦 Gender: ${response.gender.percentages.female}% female, ${response.gender.percentages.male}% male`);
      console.log(`🌎 Region: ${response.region.percentages.latam}% LATAM, ${response.region.percentages.non_latam}% non-LATAM`);
      console.log(`💼 Experience: ${response.experience.percentages.junior}% junior, ${response.experience.percentages.mid}% mid, ${response.experience.percentages.senior}% senior`);
  ...
  async testTimelineEndpoint(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Success: ${response.metadata.totalSubmissions} submissions over ${response.timeline.length} time periods`);
  ...
  async testReviewMetricsEndpoint(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Success: ${response.completedEvaluations} completed reviews`);
      console.log(`⏱️  Average review time: ${response.averageTimePerReview} minutes`);
      console.log(`🕒 Total review effort: ${response.totalReviewHours} hours`);
      console.log(`📊 Score distribution: ${Object.entries(response.scoreDistribution).map(([score, count]) => `${score}pts: ${count}`).join(', ')}`);
  ...
  private async validateEnvironment(): Promise<void> {
    ...
    if (appCount < 5) {
      console.log(`⚠️  Warning: Only ${appCount} applications found. Analytics require minimum 5 for aggregation.`);
    } else {
