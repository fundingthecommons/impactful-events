...
async function main() {
  ...
  const exampleConf = await prisma.event.upsert({
    ...
    update: {
      ...
      description: 'A showcase conference demonstrating all platform features — schedule management, speaker applications, project tracking, community engagement, and more.',
  ...
  console.log(`✅ Created/updated event: ${exampleConf.name} (${exampleConf.slug})`)
  ...
  for (const u of demoUsers) {
    ...
    console.log(`  ✅ ${u.firstName} ${u.surname} — ${u.jobTitle}, ${u.company}`)
  }
  ...
  const confSessions: ConfSessionDef[] = [
    ...
    },
    {
      ...
      description: 'A closing address weaving together the themes of the conference — funding, building, governing, and measuring public goods. An invitation to keep building after we leave San Francisco.',
  ...
  const projectsData = [
    ...
    },
    {
      ...
      updates: [
        ...
        { title: 'Dashboard and API launch', content: 'Launched the public dashboard at veridichain.demo — anyone can now explore carbon credit lifecycle data, verify measurement provenance, and track retirement status in real time.\n\nAlso shipped a REST API for third-party integrations. Documentation is live.', tags: ['demo', 'milestone'], weekNumber: 3 },
      ],
    },
    {
      ...
      updates: [
        { title: 'Bridge contract deployed on testnet', content: 'The core bridge contract is live on Optimism Sepolia and Arbitrum Sepolia. Successfully tested end-to-end attestation bridging — an attestation created on OP can now be verified on Arbitrum within 2 blocks.\n\nGas costs are reasonable: ~0.002 ETH per bridge operation.', tags: ['milestone', 'demo'], weekNumber: 1 },
    ...
    },
    {
      ...
      title: 'Web3UI Kit',
      description: 'A comprehensive React component library for Web3 applications. Wallet connection, token displays, governance interfaces, and transaction builders — all with built-in accessibility and dark mode.',
  ...
  const asksOffersData = [
    ...
    { email: 'felix.braun@demo.example', type: 'OFFER' as const, title: 'Infrastructure setup for open-source projects', description: 'I can help set up CI/CD pipelines, Docker configurations, and monitoring for your open-source project. Takes about 2-3 hours per project — free for public goods builders.', tags: ['infrastructure', 'devops', 'mentorship'] },
  ...
  const praiseMessages = [
    { senderEmail: 'amara.okafor@demo.example', recipientEmail: 'elena.vasquez@demo.example', message: 'Elena did an incredible UX review of GovGraph — her feedback completely changed how we think about governance data visualization. The wireframes she provided were production-ready.', category: 'mentorship' },
    ...
    { senderEmail: 'diego.morales@demo.example', recipientEmail: 'sofia.andersen@demo.example', message: 'Sofia helped me refine the product strategy for Web3UI Kit. Her product sense is sharp — she identified the exact three features that would drive adoption.', category: 'mentorship' },
  ...
  const forumThreads = [
    ...
    },
    {
      ...
      title: 'Security audit costs are pricing out small projects — what can we do?',
  ...
  const confQuestions = [
    ...
    { questionKey: 'email', questionEn: 'Email address', questionEs: 'Correo electrónico', questionType: 'EMAIL' as const, required: true, order: 2 },
    { questionKey: 'company', questionEn: 'Company or organization', questionEs: 'Empresa u organización', questionType: 'TEXT' as const, required: false, order: 3 },
    ...
    { questionKey: 'why_attend', questionEn: 'Why do you want to attend this conference?', questionEs: '¿Por qué quieres asistir a esta conferencia?', questionType: 'TEXTAREA' as const, required: true, order: 7 },
    { questionKey: 'what_contribute', questionEn: 'What can you contribute to the community?', questionEs: '¿Qué puedes aportar a la comunidad?', questionType: 'TEXTAREA' as const, required: true, order: 8 },
    { questionKey: 'terms_agreement', questionEn: 'Do you agree to the code of conduct?', questionEs: '¿Aceptas el código de conducta?', questionType: 'SELECT' as const, required: true, options: ['Yes', 'No'], order: 9 },
  ]
